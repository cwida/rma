...
TEST_CASE("sanity"){
  auto driver0 = make_shared<PMA_Impl4>();
...
TEST_CASE("insertions"){
    const size_t capacity = 1024 * 1024; // 1 MB insertions
//    int64_t* keys = new int64_t[capacity];
    auto keys_ptr = generate_array(capacity, 7);
    auto keys = keys_ptr.get();

    // Insert the elements
    PMA_Impl4 pma;
    for(int64_t i = 0; i < capacity; i++){
//        cout << "[" << i << "] Insert: " << keys[i] << "\n";
        pma.insert(keys[i].first, keys[i].second);
    }

//    pma.dump();
    REQUIRE(pma.size() == capacity);
    ...
    while(it->hasNext()){
        auto pair = it->next();
        index++;
        REQUIRE(pair.first == index);
        REQUIRE(pair.second == index * 1000);
    }
...
TEST_CASE("sum"){
    using Implementation = PMA_Impl4;
