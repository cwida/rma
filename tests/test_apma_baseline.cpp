/**
 * Collection of test cases related to the implementation of B-Tree PMA
 */
...
TEST_CASE("basic"){
    APMA_Baseline btree{4,4};
    REQUIRE(btree.size() == 0);
    REQUIRE(btree.empty());

    // insert 20 elements in the tree
    REQUIRE(btree.find(10) == -1);
    btree.insert(10, 1000);
    REQUIRE(btree.find(10) == 1000);

    REQUIRE(btree.find(20) == -1);
    btree.insert(20, 2000);
    REQUIRE(btree.size() == 2);
    REQUIRE(btree.find(20) == 2000);
    REQUIRE(btree.find(10) == 1000);
    ...
    btree.insert(1, 100);
    btree.insert(8, 800);
    btree.insert(11, 1100);
    REQUIRE(btree.size() == 20);
    REQUIRE(!btree.empty());
    for(size_t i = 1; i <= 11; i++){
        REQUIRE(btree.find(i) == i * 100);
    }
    REQUIRE(btree.find(12) == -1);

    auto it = btree.iterator();
    size_t index = 0;
    while(it->hasNext()){
        auto p = it->next();
        if(p.first <= 11 || p.first % 10 == 0){
            REQUIRE(p.second == p.first * 100);
        } else {
            REQUIRE(false);
        }
...
TEST_CASE("insert_17"){
    ...
    constexpr size_t elements_sz = 17;
    for(size_t i = 1; i <= elements_sz; i++){
        btree.insert(i, i * 10);
    }
//    btree.dump();

    REQUIRE(btree.size() == elements_sz);
    REQUIRE(!btree.empty());
    for(size_t i = 1; i <= elements_sz; i++){
        REQUIRE( btree.find(i) == i * 10 );
    }
    ...
    while(it->hasNext()){
        index++;
        auto e = it->next();
        REQUIRE(e.first == index);
        REQUIRE(e.second == index * 10);
    }
    REQUIRE(index == btree.size());
    REQUIRE(index == elements_sz);
}

TEST_CASE("sequential"){
    APMA_Baseline pma{7,7};
...
TEST_CASE("sum"){
    using Implementation = APMA_Baseline;
    shared_ptr<Implementation> implementation{ new Implementation{4, 4} };
