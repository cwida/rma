...
* As in [1], memory rewiring is performed on [huge pages](https://www.kernel.org/doc/Documentation/vm/hugetlbpage.txt). Its support may need to be explicitly enabled by a privileged user. In our environment, we set:
```bash
...
* `apma_sequential`: sequential pattern, with the keys generated as 1, 2, 3, ...  The `apma_` prefix is because it mimics one of the patterns also examined in the APMA paper [7].
...
# DRF12 [2], source code not included here, request it directly to its authors.
...
# PM14 [3], source code: pma/external/montes/*, pma/external.* (wrapper)
...
# KLS17 [4], source code: pma/external/iejoin/*, pma/external.* (wrapper)
...
# SLH17 [5], source code: pma/external/sha/*, pma/external.* (wrapper)
...
for B in 32 128 512 2048; do 
    # ART [6] / B+Tree, source code: abtree/art.* (wrapper), third-party/art/*
...
# Original APMA strategy [7], source code: pma/adaptive/bh07_v2/*
...
This section compares the top-down strategy of [2] with the bottom-up algorithm described in the paper. Again, we do not have the permission the redistribute the source code of [2], it 
...
# DFR [2], with top-down bulk loading, source code not included
...
References:
---
1. F. Schuhknecht, J. Dittrich, A. Sharma. RUMA has it: rewired user-space memory access is possible! In VLDB 2016. [Link](http://www.vldb.org/pvldb/vol9/p768-schuhknecht.pdf).
2. M. Durand, B. Raffin, and F. Faure. A packed memory array to keep moving particles sorted. In VRIPHYS, 2012. [Link](https://hal.inria.fr/hal-00762593/document).
3. P. Montes. Packed-memory array, 2014. Source code available online at http://github.com/pabmont/pma
4. Z. Khayyat, W. Lucia, M. Singh, M. Ouzzani, P. Papotti, J.-A. Quiané-Ruiz, N. Tang, and P. Kalnis. Fast and scalable inequality joins. In VLDB 2017. [Link](https://link.springer.com/article/10.1007/s00778-016-0441-6), [Source code](https://drive.google.com/file/d/1fqy1uC-CcYpZSvOGZOq3uoT4hg3kdqfp/view?usp=sharing).
5. M. Sha, Y. Li, B. He, and K.-L. Tan. Accelerating dynamic graph analytics on gpus. VLDB, 2017. [Link](https://www.vldb.org/pvldb/vol11/p107-sha.pdf), [Source code](https://github.com/desert0616/gpma_bfs_demo/blob/master/cpu_baseline/containers/pma_dynamic_graph.hpp).
6. V. Leis, A. Kemper, and T. Neumann. The adaptive radix tree: Artful indexing for main-memory databases. In ICDE, 2013. [Link](https://ieeexplore.ieee.org/document/6544812), [Source code](http://github.com/flode/ARTSynchronized).
7. M. Bender and H. Hu. An adaptive packed-memory array. In TODS, 2007. [Link](https://dl.acm.org/citation.cfm?doid=1292609.1292616).
8. T. Bingmann. STX B+ tree C++ template classes v0.9, 2013. Available online at: http://github.com/bingmann/stx-btree
