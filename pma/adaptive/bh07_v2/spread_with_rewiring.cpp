...
SpreadWithRewiring::SpreadWithRewiring(APMA_BH07_v2* instance, size_t window_start, size_t window_length, const VectorOfPartitions& partitions)
...
void SpreadWithRewiring::spread_elements_left2right(int64_t* __restrict destination_keys, int64_t* __restrict destination_values, size_t extent_id){
    ...
    for(size_t i = 0; i < m_segments_per_extent; i+=2){
        ...
        while(output_run_sz > 0){
            ...
            size_t input_copied;
            if(m_insert_segment == input_segment_id || m_insert_segment == input_segment_id +1 || input_segment_id >= m_window_start + m_window_length){ // merge manually
