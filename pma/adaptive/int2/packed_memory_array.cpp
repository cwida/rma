...
//    constexpr bool use_rewiring = false;
    const bool is_spread_with_insert = is_spread && is_insert;
    ...
    if(is_spread && is_insert){
        if(!use_rewiring){ // this is only for the traditional ::spread
    ...
    if(is_spread_with_insert){ m_storage.m_segment_sizes[segment_id]--; }

    if(weights.empty() && is_spread && !use_rewiring){ // shortcut, traditional strategy for spread
        PROFILER_APMA_STOP
        ...
        if(is_spread && window_length != m_storage.m_number_segments){
            mdi.resize(weights.size() *2);
            ptr_mdi = &mdi;
        }

        AdaptiveRebalancing ar{ *this, move(weights), wbalance, ar_window_length, ar_cardinality, ptr_mdi, true };
        ...
        if(is_spread) {
            PROFILER_SPREAD_START
            if(use_rewiring){
                COUT_DEBUG("-- SPREAD WITH REWIRING --");
                SpreadWithRewiring rewiring_instance{this, (size_t) window_start, (size_t) window_length, partitions};
                if(is_insert) rewiring_instance.set_element_to_insert(*key, *value, segment_id);
 ...
 *                                                                           *
 *****************************************************************************/
void PackedMemoryArray::resize(const VectorOfPartitions& partitions, bool is_insert){
    // use rewiring?
    if(is_insert && m_storage.m_memory_keys != nullptr && m_storage.m_number_segments * m_storage.m_segment_capacity * sizeof(int64_t) >= m_storage.m_memory_keys->get_extent_size()){
        resize_rewire(partitions);
    } else { // standard method
        resize_general(partitions, is_insert);
    }

    // rebuild the detector
    m_detector.resize(m_storage.m_number_segments);

    // side effect: regenerate the thresholds
    thresholds(m_storage.m_height, m_storage.m_height);
}

void PackedMemoryArray::resize_rewire(const VectorOfPartitions& partitions){
...
void PackedMemoryArray::resize_general(const VectorOfPartitions& partitions, const bool is_insert) {
...
pair<double, double> PackedMemoryArray::thresholds(int height) {
    return thresholds(height, m_storage.m_height);
}

pair<double, double> PackedMemoryArray::thresholds(int node_height, int tree_height) {
...
size_t PackedMemoryArray::memory_footprint() const {
    ...
    return sizeof(decltype(*this)) + space_index + space_elts + space_cards + space_detector;
}
...
unique_ptr<::pma::Iterator> PackedMemoryArray::empty_iterator() const{
    return make_unique<pma::adaptive::int2::Iterator>(m_storage);
}

unique_ptr<::pma::Iterator> PackedMemoryArray::find(int64_t min, int64_t max) const {
    if(empty()) return empty_iterator();
    return make_unique<pma::adaptive::int2::Iterator> (m_storage, m_index.find_first(min), m_index.find_last(max), min, max );
}
unique_ptr<::pma::Iterator> PackedMemoryArray::iterator() const {
    if(empty()) return empty_iterator();
    return make_unique<pma::adaptive::int2::Iterator> (m_storage, 0, m_storage.m_number_segments -1,
