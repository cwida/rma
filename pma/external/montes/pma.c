...
static const uint8_t max_sparseness = 4; //1 / p_0;
static const uint8_t largest_empty_segment = 4; //1 * max_sparseness;
...
static void clear_array(PMA pma);
...
PMA pma_create () {
  ...
  pma->array = (pma_element_t *)malloc (sizeof (pma_element_t) * pma->m);
  clear_array(pma);
  return (pma);
}
...
static void rebalance (PMA pma, int64_t i) {
  ...
  if (density >= p_height && density < t_height) {
    ...
    // !!! DEBUG ONLY !!!
    #if !defined(NDEBUG)
    uint64_t capacity_new = pma->m;
    if(capacity_old != capacity_new){
        printf("[pabmont " __FILE__" @ %d] Density: %f, resizing: %" PRIu64 " => %" PRIu64 " \n",
                __LINE__, density, capacity_old, capacity_new);
    }
    #endif
...
static void resize (PMA pma) {
  ...
  pma->delta_t = (t_0 - t_h) / pma->h;
  pma->delta_p = (p_h - p_0) / pma->h;
  pma->array = (pma_element_t *)realloc (pma->array, sizeof (pma_element_t) * pma->m);
...
static void clear_array(PMA pma){
    pma_element_t* __restrict array = pma->array;
    for(uint64_t i = 0, sz = pma->m; i < sz; i++){
        keyval_clear(array + i);
    }
}
