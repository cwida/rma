...
Iterator::Iterator(const Storage& storage) : m_storage(storage) { } // empty iterator

Iterator::Iterator(const Storage& storage, size_t segment_start, size_t segment_end, int64_t key_min, int64_t key_max) : m_storage(storage){
...
void Iterator::next_sequence() {
    ...
    if(segment1 < m_storage.m_number_segments){
        bool segment_even = segment1 % 2 == 0;
        if(segment_even){
            m_offset = segment1 * m_storage.m_segment_capacity + m_storage.m_segment_capacity - m_storage.m_segment_sizes[segment1];
            auto segment2 = segment1 +1;
            m_stop = segment2 * m_storage.m_segment_capacity;
            if(segment2 < m_storage.m_number_segments){
                m_stop = min(m_stop + m_storage.m_segment_sizes[segment2], m_index_max);
            } else {
            ...
            m_offset = segment1 * m_storage.m_segment_capacity;
            m_stop = min(m_index_max, m_offset + m_storage.m_segment_sizes[segment1]);
            m_next_segment++;
        }
    }
}
