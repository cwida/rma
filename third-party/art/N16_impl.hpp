...
namespace ART_unsynchronized {
    ...
    N16::N16(const uint8_t *prefix, uint32_t prefixLength) : N(NTypes::N16, prefix,
                                                                          prefixLength) {
        memset(keys, 0, sizeof(keys));
        memset(children, 0, sizeof(children));
        _private_num_nodes_N16++; // to compute the memory footprint
    }

    N16::~N16(){
        assert(_private_num_nodes_N16 > 0);
        _private_num_nodes_N16--;
    }
    ...
    void N16::copyTo(N4 *n) const {
        ...
        memcpy(n->children, children, sizeof(uintptr_t) * count);
        memset(n->children + count, 0, sizeof(uintptr_t) * (N16_shrink - count));
    }
    ...
    N* N16::getChildLessOrEqual(uint8_t key_unsigned, bool& out_exact_match) const {
        assert(count > 0 && "Empty node!");

        uint8_t key_signed = flipSign(key_unsigned);
        auto lhs = _mm_set1_epi8(key_signed);
        auto rhs = _mm_loadu_si128(reinterpret_cast<const __m128i *>(keys));
        auto cmp = _mm_cmplt_epi8(lhs, rhs);
        unsigned mask = (1U << count);
        unsigned bitfield = _mm_movemask_epi8(cmp) | mask;
        assert(bitfield && "Expected due to the OR with (1 << count)");
        unsigned index = ctz(bitfield);
        if(index > 0){
            index--;
            out_exact_match = ( keys[index] == key_signed );
